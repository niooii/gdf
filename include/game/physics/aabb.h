...
{
    return vec3_new(
        (a->min.x + a->max.x) / 2, (a->min.y + a->max.y) / 2, (a->min.z + a->max.z) / 2);
}
...
FORCEINLINE vec3 aabb_get_intersection_resolution(
    AxisAlignedBoundingBox* a, AxisAlignedBoundingBox* b)
{
    vec3 resolution = { 0 };
