...
bool terrain_pipeline_init(const GDF_VkRenderContext* vk_ctx, WorldRenderer* world_renderer)
{
    ...
    GDF_VkBufferCreateStorage((void*)STATIC_BLOCK_LOOKUP_TABLE, STATIC_BLOCK_LOOKUP_TABLE_SIZE,
    ...
    VK_ASSERT(vkCreateDescriptorSetLayout(vk_ctx->device.handle, &layout_create_info,
        vk_ctx->device.allocator, &pipeline->descriptor_layout));
    ...
    VK_ASSERT(vkCreateDescriptorPool(
        vk_ctx->device.handle, &pool_info, vk_ctx->device.allocator, &pipeline->descriptor_pool));
    ...
    for (int i = 0; i < image_count; i++)
    {
        ...
        VK_ASSERT(vkAllocateDescriptorSets(
            vk_ctx->device.handle, &set_alloc_info, &pipeline->descriptor_sets[i]));
    }

    // Update sets
    // Fragment texture sampler
    VkDescriptorImageInfo image_info = { .sampler = world_renderer->block_textures.sampler,
        .imageView   = world_renderer->block_textures.texture_array.view,
        .imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL };
    // SSBO for static block data for gpu lookups
    VkDeviceSize           offset      = { 0 };
    VkDescriptorBufferInfo buffer_info = {
        .buffer = pipeline->block_lookup_ssbo.handle, .offset = offset, .range = VK_WHOLE_SIZE
    };
    ...
    for (u32 i = 0; i < image_count; i++)
    {
        ...
        vkUpdateDescriptorSets(vk_ctx->device.handle,
            sizeof(descriptor_writes) / sizeof(VkWriteDescriptorSet), descriptor_writes, 0, NULL);
    }

    // Vertex input configuration
    VkVertexInputBindingDescription bindings = {
        .binding = 0, .stride = sizeof(ChunkVertex), .inputRate = VK_VERTEX_INPUT_RATE_VERTEX
    };
    ...
    VkDescriptorSetLayout descriptor_layouts[2] = { vk_ctx->vp_ubo_layout,
    ...
    VK_ASSERT(vkCreatePipelineLayout(vk_ctx->device.handle, &layout_info, vk_ctx->device.allocator,
    ...
    // into pipelines
    VkDynamicState d_states[2] = { VK_DYNAMIC_STATE_VIEWPORT, VK_DYNAMIC_STATE_SCISSOR };
    ...
    VK_ASSERT(vkCreateGraphicsPipelines(vk_ctx->device.handle, VK_NULL_HANDLE, 1,
        &pipeline_create_info, vk_ctx->device.allocator, &pipeline->handle));
    pipeline_create_info.layout  = pipeline->wireframe_layout;
    rasterizer_state.polygonMode = VK_POLYGON_MODE_LINE;
    VK_ASSERT(vkCreateGraphicsPipelines(vk_ctx->device.handle, VK_NULL_HANDLE, 1,
        &pipeline_create_info, vk_ctx->device.allocator, &pipeline->wireframe_handle));
...
//         VK_COLOR_COMPONENT_B_BIT | VK_COLOR_COMPONENT_A_BIT, .blendEnable = VK_TRUE,
//         .srcColorBlendFactor = VK_BLEND_FACTOR_SRC_ALPHA,
//         .dstColorBlendFactor = VK_BLEND_FACTOR_ONE_MINUS_SRC_ALPHA,
//         .colorBlendOp = VK_BLEND_OP_ADD,
//         .srcAlphaBlendFactor = VK_BLEND_FACTOR_ONE,
