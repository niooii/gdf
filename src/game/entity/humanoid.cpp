...
void HumanoidMovementController::OnGround::react(
    const HumanoidStateChangeEvent& action, EventControl& control)
{
...
void HumanoidMovementController::InAir::update(FullControl& control)
{
    ...
    //     if (!just_jumped)
    dv = vec3_mul_scalar(dv,
        AIR_CONTROL *
            0.5 /* some stupid hard coded constant HOW DO I DO GOOD AIR CONTROL?? CHAT? */);

    vec3_add_to(&vel->vec, dv);
}

void HumanoidMovementController::InAir::react(
    const HumanoidStateChangeEvent& action, EventControl& control)
{
    if (action.action_bits.has_bits(HumanoidActionBit::Dash) && control.context()->dash_available)
        control.changeTo<Dashing>();
...
void HumanoidMovementController::Dashing::enter(Control& control)
{
    ...
    else
    {
        ...
        context->dash_dir = vec3_add(
            vec3_mul_scalar(right, context->x_input), vec3_mul_scalar(forward, context->z_input));
