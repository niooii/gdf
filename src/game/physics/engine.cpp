...
void PhysicsSimulation::update(f32 dt)
{
    ...
    for (auto [entity, vel_comp, collider] : view.each())
    {
        ...
        vec3 deltas = vec3_new(vel.x * dt + 0.5f * net_accel.x * dt * dt,
            vel.y * dt + 0.5f * net_accel.y * dt * dt, vel.z * dt + 0.5f * net_accel.z * dt * dt);
        ...
        u32 results_len = world->get_blocks_touching(&t_aabb, results, std::size(results));

        // Check for floor (only if translated aabb's y is below the current one)
        GDF_BOOL ground_found        = GDF_FALSE;
        GDF_BOOL should_check_ground = t_aabb.min.y <= collider.aabb.min.y;
        f32      ground_y            = 0;
